...
public class RetryPolicyTest extends Testing {
  ...
  public void shouldNotRetry() {
    Testing.testGetSuccess(Failsafe.with(RetryPolicy.ofDefaults()), ctx -> {
      return true;
    }, (f, e) -> {
      Assert.assertEquals(e.getAttemptCount(), 1);
      Assert.assertEquals(e.getExecutionCount(), 1);
    }, true);
  }
  ...
  public void assertScheduledRetryDelay() throws Throwable {
    ...
    // Sync when / then
    Testing.ignoreExceptions(() -> Failsafe.with(rp).run(() -> {
      throw new IllegalStateException();
    }));
    waiter.await(1000);
