 ...
 */
@Test
public class InterruptionTest extends Testing implements RetryPolicyTesting {
  /**
  ...
  public void shouldThrowWhenInterruptedDuringSynchronousExecution() {
    ...
    assertThrows(() -> Failsafe.with(RetryPolicy.builder().withMaxRetries(0).build()).run(() -> {
      try {
        Thread.sleep(10000);
      } catch (InterruptedException e) {
  ...
  public void shouldThrowWhenInterruptedDuringRetryPolicyDelay() {
    ...
    assertThrows(() -> Failsafe.with(rp).run(() -> {
      throw new Exception();
    }), FailsafeException.class, InterruptedException.class);
    // Clear interrupt flag
