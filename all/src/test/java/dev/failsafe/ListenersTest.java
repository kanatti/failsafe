...
public class ListenersTest extends Testing {
  ...
  private void assertForUnhandledFailure(boolean sync) throws Throwable {
    ...
    // When
    if (sync)
      assertThrows(() -> failsafe.get(supplier), IllegalArgumentException.class);
    else
      assertThrows(() -> failsafe.getAsync(supplier).get(), ExecutionException.class,
  ...
  private void assertForRetriesExceeded(boolean sync) throws Throwable {
    ...
    // When
    if (sync)
      assertThrows(() -> failsafe.get(supplier), IllegalStateException.class);
    else
      assertThrows(() -> failsafe.getAsync(supplier).get(), ExecutionException.class,
  ...
  private void assertForFailingRetryPolicy(boolean sync) throws Throwable {
    ...
    // When
    if (sync)
      ignoreExceptions(() -> failsafe.get(supplier));
    else
      ignoreExceptions(() -> failsafe.getAsync(supplier));
